...
if($pracovisko != null || $meno!=null) {
    if ($pracovisko != null && $meno==null)
        $sql = mysqli_query($con, "select * from pozícia join(pracovisko join (login join (uzivatel join os_udaje on uzivatel.OS_udaje_idOS_udaje =os_udaje.idOS_udaje) on Login.idLogin =uzivatel.Login_idLogin)ON Login.Pracovisko_idPracovisko = pracovisko.idPracovisko)on login.Pozícia_idPozícia=pozícia.idPozícia where  LOWER(Názov) like LOWER(\"%" . $pracovisko . "%\")");
    if ($meno != null && $pracovisko==null)
        $sql = mysqli_query($con, "select * from pozícia join(pracovisko join (login join (uzivatel join os_udaje on uzivatel.OS_udaje_idOS_udaje =os_udaje.idOS_udaje) on Login.idLogin =uzivatel.Login_idLogin)ON Login.Pracovisko_idPracovisko = pracovisko.idPracovisko)on login.Pozícia_idPozícia=pozícia.idPozícia where CONCAT(LOWER(os_udaje.Priezvisko),\" \",LOWER(os_udaje.Meno)) LIKE LOWER(\"%" . $meno . "%\") or CONCAT(LOWER(os_udaje.Meno),\" \",LOWER(os_udaje.Priezvisko)) LIKE LOWER(\"%" . $meno . "%\")");
    if ($meno != null && $pracovisko!=null)
        $sql = mysqli_query($con, "select * from pozícia join(pracovisko join (login join (uzivatel join os_udaje on uzivatel.OS_udaje_idOS_udaje =os_udaje.idOS_udaje) on Login.idLogin =uzivatel.Login_idLogin)ON Login.Pracovisko_idPracovisko = pracovisko.idPracovisko)on login.Pozícia_idPozícia=pozícia.idPozícia where (CONCAT(LOWER(os_udaje.Priezvisko),\" \",LOWER(os_udaje.Meno)) LIKE LOWER(\"%" . $meno . "%\") or CONCAT(LOWER(os_udaje.Meno),\" \",LOWER(os_udaje.Priezvisko)) LIKE LOWER(\"%" . $meno . "%\")) and  LOWER(Názov) like LOWER(\"%" . $pracovisko . "%\")");
}
else{
    $sql = mysqli_query($con, "select * from pozícia join(pracovisko join (login join (uzivatel join os_udaje on uzivatel.OS_udaje_idOS_udaje =os_udaje.idOS_udaje) on Login.idLogin =uzivatel.Login_idLogin)ON Login.Pracovisko_idPracovisko = pracovisko.idPracovisko)on login.Pozícia_idPozícia=pozícia.idPozícia");
}
$i = 0;
if($sql!=null) {
